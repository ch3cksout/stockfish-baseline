...
echo "run: $0, combining Unscaled+Scaled TCs, (4*3/2)x200x2 games, cross-validating runs"
...
#name="Stockfish_Ver16_Skill=5_st=0.100sec": localhost/CCRL scaled time, combined w/'timemargin=10'
...
cutechess-cli -repeat -rounds 200 -games 2 -tournament round-robin \
                ...
                -engine tc=1/0.100+0.100 name=stockfish_16-SL5_acherm_handicapped_100ms_Hash256 cmd=/opt/stockfish/16/avx2/stockfish-ubuntu-x86-64-avx2 option."Skill Level=5" \
                	option.Threads=1 \
                	option.Hash=256 \
                	option.EvalFile=/var/chess/nn-5af11540bbfe.nnue option."Use NNUE=true" \
                -engine st="$Rescaled_100ms" name="Stockfish_Ver16_Skill=5_st=0.100sec" cmd=/opt/stockfish/16/avx2/stockfish-ubuntu-x86-64-avx2 option."Skill Level=5" \
                	...
                	option.Hash=16 \
                	option.EvalFile=/var/chess/nn-5af11540bbfe.nnue option."Use NNUE=true" \
                -engine st="$Rescaled_100ms" name="Stockfish_Ver16_Skill=5_st=0.100sec_Hash=256" cmd=/opt/stockfish/16/avx2/stockfish-ubuntu-x86-64-avx2 option."Skill Level=5" \
                	option.Threads=1 \
                	option.Hash=256 \
                	option.EvalFile=/var/chess/nn-5af11540bbfe.nnue option."Use NNUE=true" \
