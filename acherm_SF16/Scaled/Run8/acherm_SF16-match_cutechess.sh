...
echo "run: $0, 1x500x2 games, cross-validating runs"
...
cutechess-cli -repeat -rounds 500 -games 2 -tournament round-robin \
                ...
                -engine st="$Rescaled_100ms" name="Stockfish_Ver16_Skill=4_st=0.100sec" cmd=/opt/stockfish/16/avx2/stockfish-ubuntu-x86-64-avx2 option."Skill Level=6" \
                	option.Threads=1 \
                	option.Hash=16 \
                	option.EvalFile=/var/chess/nn-5af11540bbfe.nnue option."Use NNUE=true" \
